...
Yuen and Shapiro @Yuen1978 first proposed encoding information in a squeezed-quadrature field to surpass the classical Shannon limit imposed by coherent-state (shot-noise) channels.  By modulating the squeezed quadrature and homodyning at the receiver, one can achieve a signal-to-noise ratio increased by a factor $e^(2r)$ over a coherent-state channel for the same mean photon number . Thus we can lock onto the squeezed quadrature to achieve a higher signal to noise ratio, and the noise can be put into the other quadrature.
